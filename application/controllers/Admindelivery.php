...
class Admindelivery extends CI_Controller {
	...
	{ 
		...
		// echo $this->encryption->decrypt($_SESSION['adminuserid']);
	}
		 ...
		 public function getdelivery(){
		 	...
		 	if($_SESSION['adminusertp'] == 'admin'){
		 		$a['tabledata'] = $this->delivery_modal->get_users();
		 		 ...
		 		 $a['tabledata'] = $this->delivery_modal->get_usersbyuserid($usrid);
		 ...
		 public function dboyassign(){
		  ...
		  public function deliverystatus(){
		 	...
		 	$res = $this->delivery_modal->deliverystatus($usrid,$id,$dat,$tim);
        ...
        $message = "Dear ".$user_name." Thank you for shopping with us, Your order no ".$order_id." has been Delivered!";
		  ...
		  public function shippedstatus(){
		 	...
		 	$tim=$this->input->post('tim');
		 	$order_id=$this->input->post('order_id');
		 	$res = $this->delivery_modal->shippedstatus($usrid,$id,$dat,$tim);
...
//start
$pdf->SetTitle("Online Shop-Invoice");
...
$useradd = $serrow->user_address.','.$serrow->user_city.','.$serrow->user_pincode;
 ...
 //  	}
  	if($row->order_status == 0 && $row->dc_cancel_order == 1){
                    ...
                    if($row->order_status == 1 && $row->dc_cancel_order == 1){
                      ...
                      if($row->dc_cancel_order == 0 ){
                        $subtotal = $subtotal + $row->dc_prod_actualstoreprice ;
                        $deliverycharge = $row->dc_deliveryboy_charge + $row->dc_deliveryowner_charge ;
                      }
...
$html .= '
<tr>
		<td align="right" colspan="5"><b>Delivery Charge (Rs.)  '.number_format($deliverycharge,2).'</b></td>
</tr>
<tr>

                <td align="right" colspan="5"><b>Total Amount (Rs.)  '.number_format($subtotal+$deliverycharge,2).'</b></td>
