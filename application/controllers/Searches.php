...
class Searches extends CI_Controller {
	...
	public function index()
	{
		...
		$lat = $this->input->get('place1');
		$lon = $this->input->get('place2');
		...
		// 		$data ['getproducts']=  $this->display_modal->getproductsbyidcategory($id,$strid,'8','latest');
			
			
		// 	$data['categorybrand'] = $this->display_modal->getcat_brandsbycatid($id,$strid);
		// }else{
		// 	if(!empty($sid)){
		// 		$data ['getproducts']=  $this->display_modal->getproductsbyidsubcategory($sid,$strid,'8','latest');
		...
		// 		$data['getproducts'] = $this->display_modal->getbrandsbyid($bid,$strid,'8','latest');
		...
		// $data['subids'] = $sid;
		// $data['bid'] = $bid;
		$data['lats'] = base64_encode($this->input->get('place1'));
		$data['lons'] = base64_encode($this->input->get('place2'));
		$data['searchitem'] = $this->input->get('searchs');
		$searchitem = $this->input->get('searchs');
		...
		$data ['content'] =  'searchproduct_view';
		$data ['getproducts']=  $this->display_modal->getsearches($searchitem,$strid,'8','latest');
		if(!empty($data ['getproducts'])){
			$this->load->view('grocerytemplate',$data);
		}else{
			$data ['content'] =  'displayerrorpage_view';
			$this->load->view('grocerytemplate',$data);
		}
...
// echo distance(32.9697, -96.80322, 29.46786, -98.53506, "N") . " Nautical Miles<br>";
	...
	public function getsearches(){
		...
		// $offer = $this->input->post('offer');
			$a ['getfoods']=  $this->display_modal->getsearches($searchitem,$strid,$limits,$sorts);
