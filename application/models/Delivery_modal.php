...
class Delivery_modal extends CI_Model 
{
	...
	function displaydelivery()
	{
		...
		$selqry = "SELECT * FROM `deliverycharge` LEFT JOIN cart ON deliverycharge.dc_cart_id = cart.cart_id LEFT JOIN product ON cart.cart_product_id = product.prod_id GROUP BY deliverycharge.dc_id ORDER BY dc_date DESC";
		$query = $this->db->query($selqry);
        // $this->db->where('user_name !=',$emailid);
        // $this->db->where('user_name',$userid);
        // // $this->db->or_where('phone_number',$username);
	...
	public function get_usersbyuserid($userid)
	{
		...
		$selqry = "SELECT * FROM `deliverycharge` LEFT JOIN cart ON deliverycharge.dc_cart_id = cart.cart_id LEFT JOIN product ON cart.cart_product_id = product.prod_id LEFT JOIN user ON cart.cart_user_id = user.user_id LEFT JOIN address ON address.address_id = deliverycharge.dc_address_id LEFT JOIN firebase_user ON cart.cart_user_id = firebase_user.firebase_user_id WHERE deliverycharge.dc_agent_id = '$userid' AND deliverycharge.dc_cancel_order = 0 GROUP BY deliverycharge.dc_order_id ORDER BY dc_date,dc_time DESC";
...
public function get_usersbyuseridforhome($userid)
	{
		...
		$selqry = "SELECT * FROM `deliverycharge` LEFT JOIN cart ON deliverycharge.dc_cart_id = cart.cart_id LEFT JOIN product ON cart.cart_product_id = product.prod_id LEFT JOIN user ON cart.cart_user_id = user.user_id  WHERE deliverycharge.dc_agent_id = '$userid'  AND deliverycharge.dc_status = 1 AND deliverycharge.order_status = 1 GROUP BY deliverycharge.dc_order_id ORDER BY dc_date,dc_time DESC";
	...
	public function get_users()
	{
		...
		$selqry = "SELECT * FROM `deliverycharge` LEFT JOIN cart ON deliverycharge.dc_cart_id = cart.cart_id LEFT JOIN product ON cart.cart_product_id = product.prod_id LEFT JOIN user ON cart.cart_user_id = user.user_id LEFT JOIN address ON deliverycharge.dc_address_id = address.address_id  GROUP BY deliverycharge.dc_order_id ORDER BY dc_date,dc_time DESC";
		$query = $this->db->query($selqry);
...
public function get_usersforadmin()
	{
		...
		// $selqry = "SELECT * FROM `deliverycharge` ";
		$selqry = "SELECT * FROM `deliverycharge` LEFT JOIN cart ON deliverycharge.dc_cart_id = cart.cart_id LEFT JOIN product ON cart.cart_product_id = product.prod_id LEFT JOIN user ON cart.cart_user_id = user.user_id WHERE deliverycharge.dc_status = 1 AND deliverycharge.order_status = 1 GROUP BY deliverycharge.dc_order_id ORDER BY dc_date,dc_time DESC";
		$query = $this->db->query($selqry);
	...
	public function get_deliverymulitpleall($id,$dat,$tim,$filt)
	{
	    ...
	    $selqry = "SELECT * FROM `deliverycharge` LEFT JOIN cart ON deliverycharge.dc_cart_id = cart.cart_id LEFT JOIN product ON cart.cart_product_id = product.prod_id LEFT JOIN user ON cart.cart_user_id = user.user_id WHERE deliverycharge.dc_order_id = '$id' and dc_time = '$tim' and dc_date = '$dat' ".$filt."GROUP BY deliverycharge.dc_id ORDER BY deliverycharge.dc_date DESC";
	...
	public function deliverystatus($usid,$id,$dat,$tim)
	{
	...
	public function shippedstatus($usid,$id,$dat,$tim)
	{
		//$this->db->where('dc_agent_id',$usid);
	    $this->db->where('dc_user_id',$id);
	    $this->db->where('dc_time',$tim);
	...
	public function get_delivery($userid)
	{
	    ...
	    $selqry = "SELECT * FROM `deliverycharge` LEFT JOIN cart ON deliverycharge.dc_cart_id = cart.cart_id LEFT JOIN product ON cart.cart_product_id = product.prod_id LEFT JOIN user ON cart.cart_user_id = user.user_id WHERE user.user_id = '$userid' GROUP BY deliverycharge.dc_date,deliverycharge.dc_time ORDER BY deliverycharge.dc_date DESC";
		$query = $this->db->query($selqry);
         return $query->result();   
	}
	// new one end
